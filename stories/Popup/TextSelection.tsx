...
import { PopperVirtualElement, Popup } from "@lilib/ui";
...
function Example() {
  ...
  const [getSelection] = useTimeout(() => {
    ...
    if (selection && selection.rangeCount > 0) {
      ...
      if (range && !range.collapsed && containerRef.current?.contains(range.commonAncestorContainer)) {
  ...
  const virtualElement: PopperVirtualElement = {
    getBoundingClientRect() {
      return rangeRef.current!.getClientRects()[0] || rangeRef.current!.getBoundingClientRect();
    },
  };
  ...
  return (
    <>
      ...
      <Popup content="This is a popup message" open={open} placement="top-start" onClose={() => setOpen(false)}>
        {() => virtualElement}
      </Popup>
    </>
  );
