...
This component is used to listen for events of anchor and layer elements, and triggers `onOpen` and `onClose` handlers. You can use these handlers to display your UI elements. This component does not provide any UI elements, the anchor and layer elements are provided by yourself. It is commonly used to build pop-up components, such as Popup, Modal.
...
| `on`              | `"click"` \| `"hover"` \| `"focus"` \| `"context-menu"` \| `string[]`              | Open event. Default is `"click"`.                             |
...
| Property   | Type           | Description                                    |
| ---------- | -------------- | ---------------------------------------------- |
| `children` | `ReactElement` | Trigger anchor, must references a DOM element. |

The `children` must support `onClick`, `onFocus`, `onBlur`, `onMouseEnter`, `onMouseLeave` and `onContextMenu` events.

### Trigger.Layer

| Property   | Type           | Description                                   |
| ---------- | -------------- | --------------------------------------------- |
| `children` | `ReactElement` | Trigger layer, must references a DOM element. |

The `children` must support `onMouseEnter`, `onMouseLeave`, `onMouseDown` and `onTouchStart` events.
