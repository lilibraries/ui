...
You can use the `Popper.Config` to configure the `container` prop of all subcomponents those based on the Popper component. It is very useful when building nested popup layers.

<Canvas mdxSource={require("!!raw-loader!./Config").default}>
  <Story id="examples-popper--config" />
</Canvas>
...
interface PopperData {
  ...
  placement:
    ...
    | "left-start"
    | "left-end";
  // Whether the trigger element is not in the viewport.
...
Popper.Config will inherit the upper configuration.

### Popper.Config.useConfig
