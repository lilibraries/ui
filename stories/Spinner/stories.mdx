...
It is contented when the `children` prop is renderable or the `contented` prop is `true`. In contented state, the component will perform zoom animation, and it will keep the space when it's not spinning.

<Canvas mdxSource={require("!!raw-loader!./Contented").default}>
  <Story id="examples-spinner--contented" />
</Canvas>
...
| `contented`   | `boolean`                         | Assume the `children` is renderable.  |
| `startSpaced` | `boolean` \| `number` \| `string` | Set start space from other elements.  |
| `endSpaced`   | `boolean` \| `number` \| `string` | Set end space from other elements.    |
