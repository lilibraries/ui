...
`Spinner` provides `Spinner.Config` component to set `delay` and `icon` for all sub Spinner components. The following example will show custom spinner icon after 500ms when the checkbox is on.
